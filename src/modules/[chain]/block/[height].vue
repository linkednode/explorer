  ...
  <div>
    <div v-if="isFutureBlock" class="text-center">
      <div v-if="remainingBlocks > 0">
        <div class="text-primary font-bold text-3xl my-10">#{{ target }}</div>
        <Countdown :time="estimateTime" css="md:!text-5xl font-sans md:mx-5" />
        <div class="my-5 text-base-content">
          {{ $t('block.estimated_time') }}:
          <span class="text-xl font-bold text-base-content">{{ format.toLocaleDate(estimateDate) }}</span>
        </div>
        <div class="pt-10 flex justify-center">
          <table class="table w-max rounded-box shadow-md bg-base-100 text-base-content">
            <tbody>
              <tr class="hover:bg-base-200 cursor-pointer" @click="edit = !edit">
              ...
              <tr v-if="edit">
                <td colspan="2" class="text-center">
                  <h3 class="text-lg font-bold text-base-content">{{ $t('block.countdown_for_block_input') }}</h3>
                  <div class="py-4">
                    <div class="join">
                      <input class="input input-bordered join-item" v-model="newHeight" type="number" />
                      <button class="btn btn-primary join-item text-primary-content" @click="updateTarget()">
                        {{ $t('block.btn_update') }}
                      </button>
                    </div>
                  </div>
    ...
    <div v-else>
      <div class="bg-base-100 shadow-md rounded-box px-4 pt-3 pb-4 mb-4">
        <h2 class="text-xl font-semibold flex flex-row justify-between text-base-content">
          <p class="">#{{ current.block?.header?.height }}</p>
          <div class="flex" v-if="props.height">
            <RouterLink
              :to="`/${store.blockchain.chainName}/block/${height - 1}`"
              class="btn btn-primary btn-sm p-1 text-2xl mr-2 rounded-btn text-primary-content"
            >
              <Icon icon="mdi-arrow-left" class="w-full h-full" />
            </RouterLink>
            <RouterLink
              :to="`/${store.blockchain.chainName}/block/${height + 1}`"
              class="btn btn-primary btn-sm p-1 text-2xl rounded-btn text-primary-content"
            >
